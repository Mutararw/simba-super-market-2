        ...
        <div className="shop-layout section-padding">
          <aside className="categories-sidebar">
            ...
            <div className="categories-bar sidebar-categories">
              {categories.map((cat) => (
                <button key={cat} onClick={() => setCategory(cat)} className={`category-pill ${category === cat ? 'is-active' : ''}`}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <section id="products">
            ...
            {products.length === 0 ? (
              <div className="glass empty-state">
                <h3>{t.empty}</h3>
                <button className="btn-primary" onClick={() => { setCategory('All'); setSearchQuery('') }}>Clear Filters</button>
              </div>
            ) : (
